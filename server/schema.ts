...
CREATE TABLE IF NOT EXISTS issues (
  ...
  status VARCHAR(16) NOT NULL CHECK (status IN ('待处理', '处理中', '待复测', '已解决', '不适用', '待优化')),
...
SET status = CASE status
  ...
  ELSE status
END
WHERE status IN ('待验证', '已关闭');
ALTER TABLE issues ADD CONSTRAINT issues_status_check
  CHECK (status IN ('待处理', '处理中', '待复测', '已解决', '不适用', '待优化'));

UPDATE issue_activities
SET detail = REPLACE(REPLACE(detail, '待验证', '待复测'), '已关闭', '不适用')
WHERE kind = 'changed' AND action = '更新了状态'
  AND (detail LIKE '%待验证%' OR detail LIKE '%已关闭%');
`
