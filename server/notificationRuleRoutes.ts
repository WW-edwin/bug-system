...
router.get('/', async (_request, response) => {
  const [settings, { dictionaries }] = await Promise.all([loadNotificationRuleSettings(), loadDictionaries()])
  response.json({ ...settings, statuses: dictionaries.status, deliveryMode: deliveryMode() })
})
...
router.put('/', async (request, response) => {
  ...
  const result = await withTransaction(async (client) => {
    ...
    const { dictionaries } = await loadDictionaries(client)
    return { ...settings, statuses: dictionaries.status, deliveryMode: deliveryMode() }
  })
