...
第一阶段支持浏览器钉钉授权登录、已有账号首次关联和后续免输 Bug 系统密码登录。先使用指定员工的小范围试用名单；不自动创建员工，不按姓名或邮箱合并账号。第二阶段钉钉客户端内免登见 [DINGTALK_IN_APP.md](DINGTALK_IN_APP.md)。

## 账号关联方式

1. 登录页点击“钉钉登录”，整页进入钉钉官方授权页。
2. 后端以一次性授权码获取用户身份，再使用企业应用凭据确认其为本企业已激活的内部成员，并检查试用名单。
...
原有密码登录、注册和忘记密码入口仍可用。当前独立本地库没有导入生产用户；首次真实本地试用需要先通过原注册入口建立本地账号。生产账号密码不能直接用于这个空数据库。注册时的密码由使用者自行设置，不应发送到聊天或写入文档。
...
允许名单为逗号分隔的精确 userId。省略 `DINGTALK_LOGIN_ALLOWED_USER_IDS` 时回退到 `DINGTALK_TEST_USER_ID`；显式留空表示无人可试用，登录不可用。配置错误或凭据不全时，仅禁用钉钉登录，密码登录仍可用。
...
回调必须与 `PUBLIC_ORIGIN` 的第一个地址同源，路径为 `/api/auth/dingtalk/callback`。正式环境应使用正确的 HTTPS 地址与安全 Cookie。这里的 `127.0.0.1` 只用于本机浏览器登录测试；手机直接访问本机页面或钉钉内免登留待后续阶段配置可访问地址。
